...
Following the preliminary checks, each contract was reviewed individually for each market.
...
**Event Timing:** This study focused on scheduled events so forecasts could be compared at a consistent time before each event.  Polymarket’s listed end time did not always match the relevant real-world event boundary, so market rules and supporting sources were reviewed to establish the boundaries in Table 2.
...
The linked candidate ledger records the replacement attempts, decisions and rationale for the final replacement round.  Exclusion examples for each topic are listed below.
...
Individual market evidence reviews resulted in 94 sample exclusions due to eligibility or insufficient evidence (wrong event type, outside study dates, outcome determined prior to cutoff, etc.); an additional 93 were excluded due to overlapping real-world event families.  Excluded markets following individual audits were replaced using the established random reserve ordering within the same topic and month, where possible.  If no eligible candidates remained in that month, candidates were drawn from the nearest available month within the study period.  Replacement candidates passed evidence reviews before acceptance.  Exclusion rationale and replacement attempts remain recorded.
...
The final evidence reviews left 565 contracts in the cohort: 200 Sports, 200 Politics and 165 Economics, each with distinct real-world event families (Figure 1).  The preliminary and individual exclusions could introduce bias towards scheduled event markets and markets with more easily verifiable outcomes.  Replacements drawn from other months contributed to an uneven monthly distribution of Politics and Economics markets in the final sample (Figure 2).

## Cleaning & Data Quality Checks

### Repeat Trade Records

Identical returned trade rows were flagged but retained in the dataset.  One transaction may be completed via several fills with the same wallet, timestamp, price and size.  In three examined transactions, retaining repeated rows preserved BUY vs SELL transaction totals, supporting their retention.
...
Fields were added across the three tables to describe forecast timing, collection-window length, price changes and gaps, trade values/directions, transaction/wallet counts, activity totals, and wallet concentration.   Inconsistent outcome labels were mapped to consistent names using outcome token identifiers, preserving original labels.  Repeated trade rows were flagged but retained.  Missing value indicators explain missing values, including average trade size and wallet concentration when no trades were returned, and the number of event alternatives when the count could not be established.
...
![“Very large” refers to trades above the 99.9th percentile of total dollars value (price x shares). The threshold flagged 156 records across 113 transactions. A single transaction can contain multiple flagged records.  These observations are restricted to the collected pre-cutoff windows.](assets/eda_20260917_large_trades/04_large_trade_records.png){fig-alt="Dot plot of 113 transactions containing records above about $20,540. April Fed no-change: 92; July Fed 25-basis-point cut: 15; September ECB increase: 2; Arsenal, Beaudoin, McKinney and February ECB no-change: 1 each. Maximum record is $2.994 million."}
...
![The curves show how distinct wallet counts vary across markets in each topic during its collected window.  All topics demonstrate a right skew towards higher distinct wallet counts.  Counts are smoothed on a log scale, with equal area for each topic.  Multiple wallets can be held by a single trader. Markets without returned records are omitted.](assets/eda_20260917_wallet_density/08_wallet_density.png){fig-alt="Density curves for distinct wallet counts per market, with a logarithmic x-axis. Includes 136 Sports, 198 Politics and 164 Economics markets with observed wallets. Median counts among these markets are 13, 33 and 42 respectively. Zero-wallet windows are omitted."}
